...
class MainMenu extends ArcadeGame {
    ...
    setup() { this.p.background('#F4F3EF'); }
    ...
    update() {
        ...
        if (allPlayers.length > 0) {
            ...
            if (allReady) {
                // Trigger transition to next game!
                this.onStartGame();
            }
        }
    }
    ...
    draw() {
        ...
        const allPlayers = this.players.getAllPlayers();
        if (allPlayers.length === 0) {
            this.p.textSize(24); this.p.textStyle(this.p.NORMAL); this.p.fill('#888');
        ...
        allPlayers.forEach((player, index) => {
            ...
            if (player.isReady) {
                this.p.stroke('#45CB85'); this.p.strokeWeight(6); this.p.noFill();
                this.p.circle(x, y, 120);
            }

            this.p.noStroke(); this.p.fill('#2D3142'); this.p.textSize(20);
            this.p.text(player.name, x, y + 90);
            this.p.textSize(14); this.p.fill(player.isReady ? '#45CB85' : '#888');
